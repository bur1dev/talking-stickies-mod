<script lang="ts">
  ...
  // Add role store
  const currentRole = writable('scanner');

  setContext('currentRole', currentRole);

  // Add keyboard handler
  function handleKeydown(event: KeyboardEvent) {
    if (event.ctrlKey && event.key.toLowerCase() === 's') {
      event.preventDefault();
      currentRole.update(current => current === 'scanner' ? 'customer' : 'scanner');
      console.log('Switched to role:', get(currentRole));
    }
  }
  ...
  <profiles-context store={profilesStore}>
    ...
    {:else}
      {#if renderType== RenderType.CreateBoard}
        <ControllerCreate {view} {client} {weaveClient} {profilesStore} {roleName} role={currentRole}/>
      {:else if renderType== RenderType.App}
        <Controller {client} {weaveClient} {profilesStore} {roleName} role={currentRole}/>
      {:else if renderType== RenderType.Board}
        <ControllerBoard board={wal.hrl[1]} {client} {weaveClient} {profilesStore} {roleName} role={currentRole}/>
      {/if}
    {/if}
  </profiles-context>
{:else}
