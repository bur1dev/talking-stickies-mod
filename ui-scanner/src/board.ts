...
| {
  ...
  const _setGroups = (newGroups, state) => {
    ...
    // Check for Ungrouped - keep this behavior
      ...
      if (groupId != UngroupedId) {
        if (!idList[groupId]) {
          ...
          state.grouping[UngroupedId] =
            state.grouping[UngroupedId].concat(itemIds);
        }
      }
    });
  };
  ...
  export const boardGrammar = {
    initialState(init: Partial<BoardState> | undefined = undefined) {
      const state: BoardState = {
        status: "",
        name: "untitled",
        // We'll get the role from the context when needed
        groups: [{ id: UngroupedId, name: "" }], // Always initialize with Ungrouped
        grouping: undefined,
        stickies: [],
        voteTypes: [],
    ...
    applyDelta(
      ...
      switch (delta.type) {
        case "set-state":
          if (delta.state.status !== undefined)
            state.status = delta.state.status;
          ...
          if (delta.state.stickies !== undefined)
            state.stickies = delta.state.stickies;
          if (delta.state.voteTypes !== undefined)
            state.voteTypes = delta.state.voteTypes;
          if (delta.state.props !== undefined) state.props = delta.state.props;
          if (delta.state.boundTo !== undefined)
            state.boundTo = delta.state.boundTo;
        ...
        case "update-sticky-votes":
          state.stickies.forEach((sticky, i) => {
            if (sticky.id === delta.id) {
              ...
              state.stickies[i].props.votes[delta.voteType][delta.voter] =
                delta.count;
            }
          });
          break;
        case "merge-stickies":
          const srcIdx = state.stickies.findIndex(
            (sticky) => sticky.id === delta.srcId
          );
          const dstIdx = state.stickies.findIndex(
            (sticky) => sticky.id === delta.dstId
          );
          if (srcIdx >= 0 && dstIdx >= 0) {
            ...
            dst.props.attachments = cloneDeep(
              dst.props.attachments.concat(src.props.attachments)
            );
            state.stickies.splice(srcIdx, 1);
          }
          break;
        case "delete-sticky":
          const index = state.stickies.findIndex(
            (sticky) => sticky.id === delta.id
          );
