<script lang="ts">
  ...
  const newGroup = (group: uuidv1) => async () => {
    try {
      ...
      const cloneInfo = await store.service.callZome(
        "clone_cart_dna",
        cartInput
      );
  ...
  function handleDragDropCard(e: DragEvent) {
    ...
    if (target.id && srcId != target.id && confirm("Merge stickies?")) {
      board.requestChanges([
        { type: "merge-stickies", dstId: target.id, srcId },
      ]);
    }
    clearDrag();
  }
  ...
  let previousRole = $roleStore;
$: {
  if ($roleStore !== previousRole) {
    console.log("Role switched:", {
      from: previousRole,
      to: $roleStore,
      groups: $state?.groups,
      carts: $cartStore?.visibleCarts,
      filteredItems: items,
      loading: $cartStore?.loading
    });
    previousRole = $roleStore;
  }
}

$: console.log("Current computed state:", {
    role: $roleStore,
    carts: $cartStore?.visibleCarts,
    filteredItems: items,
    loading: $cartStore?.loading
    ...
    ? [...$state.groups, ...($cartStore?.visibleCarts || [])]
        .filter((group) => {
          if (group.id === UngroupedId) {
            return $roleStore === "customer";  // Show Ungrouped only in customer mode
          }
          return true;  // Show everything else (carts) always
        })
        .map((group) => ({
          id: group.id,
...
<div class="board">
  ...
  {/if}
  {#if $state}
    <div class="top-bar" class:stand-alone-top-bar={standAlone}>
          ...
          {#each sorted(group.stickyIds, sortStickies) as { id, props } (id)}
            ...
            {:else}
              <div
                ...
                style:background-color={props && props.color
                  ? props.color
                  : "#D7EEFF"}
              >
                  ...
                  {#each $state.voteTypes as { type, emoji, toolTip, maxVotes }}
                    ...
                    >
                      <div class="vote-icon-wrapper">
                        <EmojiIcon {emoji} class="vote-icon" />
                      </div>
                      {#if countVotes(props.votes, type) > 0}
                        <span class="num-votes"
                          >{countVotes(props.votes, type)}</span
                        >
                      {/if}
                      {#if maxVotes > 1}
                        <div class="vote-counts">
                          {#each [...Array(maxVotes).keys()] as index}
                            <div
                              class="vote-count"
                              class:vote-count-potential={myVotes(
                                props.votes,
                                type
                              ) <= index}
                            />
...
<style>
  ...
  .group {
    ...
    min-width: 290px;
    border: 2px dashed rgba(84, 54, 19, 0.2);
  ...
  .voted {
    border-color: rgba(110, 174, 47, 1);
    background-color: rgba(110, 174, 47, 0.7);
    box-shadow: 0 4px 5px rgba(0, 0, 0, 0.2);
    border-bottom: 2px solid rgba(70, 134, 7, 1);
  ...
  .vote-count {
    ...
    z-index: 1;
    border-radius: 10px;
    background-color: rgba(244, 200, 42, 1);
    min-width: 4px;
    min-height: 4px;
    margin: 3px;
    margin-bottom: 0px;
  ...
  .vote-count-potential {
    background-color: rgba(205, 199, 194, 1);
  }
  ...
  .voted .vote-count.background {
    background-color: rgba(70, 134, 8, 1);
  }
